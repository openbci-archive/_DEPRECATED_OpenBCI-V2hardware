/*
 ...
 This sketch has two purposes: (1) send data to the PC from the OpenBCI shield, and (2) receive
 commands from the PC to control the Hex Bug.
 
 This code builds off StreamRawData.ino, which I've been using with OpenBCI for quite a while.

 This code uses the ADS1299 Arduino Library, which is available on the OpenBCI github.  You must
 put the ADS1299 library in your Arduino "Libraries" directory.

 Created: Chip Audette, May 2014. http://eeghacker.blogspot.com
