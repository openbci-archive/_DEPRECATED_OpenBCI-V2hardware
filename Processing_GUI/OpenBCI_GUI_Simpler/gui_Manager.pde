...
//    } else {
  ...
  public void setupSpectrogram(Graph2D g, int win_x, int win_y, float[] axis_relPos,float displayTime_sec, PlotFontInfo fontInfo, TextBox title) {
    ...
    titleSpectrogram.x = (int)(g.position.x + float(win_x)*0.5f*rel_width);
    titleSpectrogram.y = (int)(g.position.y - float(win_y)*0.01f);
    titleSpectrogram.setColor(255,255,255);//make it white
    titleSpectrogram.alignX = CENTER;
    titleSpectrogram.alignY = BOTTOM;
  }
